...
- **經驗值系統**：每發送一則有效互動訊息（包含文字、圖片、影片、檔案、貼圖）可獲得 1 XP，系統會自動排除「指令」與「系統狀態更新（如成員加入、置頂）」。
- **等級公式**：採用標準 RPG 二次模型（`5L^2 + 50L + 10`），隨等級提升所需經驗。
- **自動選單**：機器人啟動後會自動設定 Telegram 指令選單，使用者無需記住複雜指令即可快速操作。
...
- `/rank`：查詢當前群組中的個人等級報告，包含等級、XP 進度與視覺化進度條。
